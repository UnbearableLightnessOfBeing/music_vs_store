...
const addImageToProduct = `-- name: AddImageToProduct :exec
UPDATE products
SET images = array_append( images, $2 )
WHERE id = $1
`

type AddImageToProductParams struct {
	ID          int32       `json:"id"`
	ArrayAppend interface{} `json:"array_append"`
}

func (q *Queries) AddImageToProduct(ctx context.Context, arg AddImageToProductParams) error {
	_, err := q.db.ExecContext(ctx, addImageToProduct, arg.ID, arg.ArrayAppend)
	return err
}
...
INSERT INTO products
  ...
  VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, name, price_int, price_dec, label_id, images, description, characteristics, in_stock
`
...
const removeImageFromProduct = `-- name: RemoveImageFromProduct :exec
UPDATE products
SET images = array_remove( images, $2 )
WHERE id = $1
`

type RemoveImageFromProductParams struct {
	ID          int32       `json:"id"`
	ArrayRemove interface{} `json:"array_remove"`
}

func (q *Queries) RemoveImageFromProduct(ctx context.Context, arg RemoveImageFromProductParams) error {
	_, err := q.db.ExecContext(ctx, removeImageFromProduct, arg.ID, arg.ArrayRemove)
	return err
}
